...
from .live_lens_accuracy import (
    ...
    compute_live_lens_ats_accuracy,
...
@app.command(name="compute-live-lens-ats-accuracy")
def compute_live_lens_ats_accuracy_cmd(
    date: str = typer.Option(None, help="Slate date YYYY-MM-DD (default: yesterday local)."),
    out_json: Path = typer.Option(None, help="Output JSON path (default: outputs/live_lens_ats_accuracy_<date>.json)."),
    out_csv: Path = typer.Option(None, help="Optional per-signal settled rows CSV output."),
    price: float = typer.Option(-110.0, help="Assumed odds price for ROI (default -110)."),
    full_game_only: bool = typer.Option(False, help="Only evaluate full-game lens (default false)."),
):
    """Compute Live Lens ATS (spread) bet ROI / win-rate from logged live signals + finalized results.
    ...
    cfg = LiveLensAccuracyConfig(date=str(date), assume_price=float(price), full_game_only=bool(full_game_only))
    payload = compute_live_lens_ats_accuracy(cfg)

    if out_json is None:
        out_json = Path("outputs") / f"live_lens_ats_accuracy_{cfg.date}.json"

    if out_csv is None:
        out_csv = Path("outputs") / f"live_lens_ats_accuracy_{cfg.date}.csv"

    wrote = write_live_lens_accuracy(out_json=Path(out_json), payload=payload, out_csv=Path(out_csv))
    print(wrote)


@app.command(name="compute-live-lens-ats-side-accuracy")
def compute_live_lens_ats_side_accuracy_cmd(
    date: str = typer.Option(None, help="Slate date YYYY-MM-DD (default: yesterday local)."),
    out_json: Path = typer.Option(None, help="Output JSON path (default: outputs/live_lens_ats_side_accuracy_<date>.json)."),
    out_csv: Path = typer.Option(None, help="Optional per-signal settled rows CSV output."),
    price: float = typer.Option(-110.0, help="Assumed odds price for ROI (default -110)."),
    full_game_only: bool = typer.Option(False, help="Only evaluate full-game lens (default false)."),
):
    """Compute Live Lens ATS (spread) accuracy by signal-side (collapsing line changes).

    If the same ATS bet signal persists while the spread moves (e.g. HOME -3.5, -4.5, -5.5),
    this collapses it to one row per (game, lens, side) using the earliest BET timestamp.
    ...
    cfg = LiveLensAccuracyConfig(date=str(date), assume_price=float(price), full_game_only=bool(full_game_only))
    payload = compute_live_lens_ats_side_accuracy(cfg)

    if out_json is None:
        out_json = Path("outputs") / f"live_lens_ats_side_accuracy_{cfg.date}.json"

    if out_csv is None:
        out_csv = Path("outputs") / f"live_lens_ats_side_accuracy_{cfg.date}.csv"
...
@app.command(name="report-live-lens-ats-side-accuracy")
def report_live_lens_ats_side_accuracy_cmd(
    date: str = typer.Option(None, help="Single slate date YYYY-MM-DD (mutually exclusive with start/end)."),
    start_date: str | None = typer.Option(None, help="Start date YYYY-MM-DD (inclusive). Defaults to end_date - days + 1."),
    end_date: str | None = typer.Option(None, help="End date YYYY-MM-DD (inclusive). Defaults to yesterday local."),
    days: int = typer.Option(5, help="Number of days to include when start_date not provided (default 5)."),
    price: float = typer.Option(-110.0, help="Assumed odds price for ROI (default -110)."),
    full_game_only: bool = typer.Option(False, help="Only evaluate full-game lens (default false)."),
    out_json: Path = typer.Option(None, help="Output JSON path (default: outputs/live_lens_ats_side_accuracy_rollup_<start>_<end>.json)."),
    out_csv: Path = typer.Option(None, help="Output CSV path (default: outputs/live_lens_ats_side_accuracy_rollup_<start>_<end>.csv)."),
):
    """Roll up Live Lens ATS side-based accuracy across multiple days.

    Uses `compute-live-lens-ats-side-accuracy` (one row per game/lens/side) per day,
    then aggregates into a single CSV + JSON summary for quick tuning decisions.
    """
    ...
    if out_json is None:
        out_json = Path("outputs") / f"live_lens_ats_side_accuracy_rollup_{start_s}_{end_s}.json"
    if out_csv is None:
        out_csv = Path("outputs") / f"live_lens_ats_side_accuracy_rollup_{start_s}_{end_s}.csv"
...
def compute_live_lens_projection_accuracy_cmd(
    date: str = typer.Option(None, help="Slate date YYYY-MM-DD (default: yesterday local)."),
    out_json: Path = typer.Option(None, help="Output JSON path (default: outputs/live_lens_projection_accuracy_<date>.json)."),
    out_csv: Path = typer.Option(None, help="Optional per-snapshot settled rows CSV output."),
    full_game_only: bool = typer.Option(False, help="Only evaluate lens=full_game (default false)."),
):
    """Compute Live Lens projected-total accuracy (MAE/RMSE) from logged projection snapshots + finalized results.

    Requires:
      - outputs/live_lens_projections_<date>.jsonl (written by the web UI)
      - outputs/daily_results/results_<date>.csv (written by finalize-day)
    """
    ...
    cfg = LiveLensProjectionAccuracyConfig(date=str(date), out_dir=settings.outputs_dir, daily_results_dir=settings.outputs_dir / "daily_results", full_game_only=bool(full_game_only))
    payload = compute_live_lens_projection_accuracy(cfg)

    if out_json is None:
        out_json = Path("outputs") / f"live_lens_projection_accuracy_{cfg.date}.json"
    if out_csv is None:
        out_csv = Path("outputs") / f"live_lens_projection_accuracy_{cfg.date}.csv"
